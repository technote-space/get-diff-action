...
const getCompareRef = (ref) => github_action_helper_1.Utils.isRef(ref) ? github_action_helper_1.Utils.getLocalRefspec(ref) : ref;
    ...
    if (github_action_helper_1.Utils.isRef(diffInfo.base) && github_action_helper_1.Utils.isRef(diffInfo.head)) {
        yield ['base', 'head'].reduce((prev, target) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            yield command.execAsync({
                command: 'git fetch',
                args: ['--no-tags', '--no-recurse-submodules', '--depth=10', 'origin', github_action_helper_1.Utils.getRefspec(diffInfo[target])],
                stderrToStdout: true,
                cwd: github_action_helper_1.Utils.getWorkspace(),
            });
        }), Promise.resolve());
        ...
        yield ['+refs/pull/*/merge:refs/pull/*/merge', '+refs/heads/*:refs/remotes/origin/*'].reduce((prev, item) => __awaiter(void 0, void 0, void 0, function* () {
            yield prev;
            yield command.execAsync({
                command: 'git fetch',
                args: ['--no-tags', '--no-recurse-submodules', '--depth=10', 'origin', item],
                stderrToStdout: true,
                cwd: github_action_helper_1.Utils.getWorkspace(),
            });
        }), Promise.resolve());
        yield command.execAsync({
            command: 'git fetch',
            args: ['--no-tags', '--no-recurse-submodules', 'origin', '+refs/heads/master:refs/remotes/origin/master'],
            stderrToStdout: true,
            cwd: github_action_helper_1.Utils.getWorkspace(),
        });
    }
