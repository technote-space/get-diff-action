...
const getDiffFiles = (diffs, filter) => misc_1.escape(diffs.filter(item => !filter || item.isMatched).map(item => item.file)).join(getSeparator());
exports.getDiffFiles = getDiffFiles;
const getMatchedFiles = (diffs) => misc_1.escape(diffs.filter(item => item.filterIgnored).map(item => item.file)).join(getSeparator());
