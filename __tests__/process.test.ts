...
describe('execute', () => {
	...
	it('should execute', async() => {
		...
		stdoutCalledWith(mockStdout, [
			...
			'  >> README.md',
			'  >> src/main.ts',
			'[command]git diff --shortstat \'package.json\'',
			'  >> 1 file changed, 25 insertions(+), 4 deletions(-)',
			'[command]git diff --shortstat \'abc/composer.json\'',
			'  >> 1 file changed, 25 insertions(+), 4 deletions(-)',
			'[command]git diff --shortstat \'README.md\'',
			'  >> 1 file changed, 25 insertions(+), 4 deletions(-)',
			'[command]git diff --shortstat \'src/main.ts\'',
			'  >> 1 file changed, 25 insertions(+), 4 deletions(-)',
			'::group::Dump output',
			'[\n' +
			'\t{\n' +
